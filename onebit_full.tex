...
\end{align}
under $\Prob_{\theta,n}$. Under the assumption that $f(x)$ is continuously differentiable with finite Fisher information for location, the model $\{Z_n+ \theta\}_{n \in \mathbb N}$ is differentiable in quadratic mean \cite[Exm. 7.8]{van2000asymptotic} and hence local asymptotically normal (LAN) in the sense that
\[
...
Any regular estimators in a LAN model satisfies \eqref{eq:attaining_LAM} \cite{beran1995role}, hence the proof of (ii) follows once we establish the regularity of $\bar{\theta}_n$. \\

%Any estimator satisfying \eqref{eq:sgd_part2} also satisfies \eqref{eq:attaining_LAM} \cite[P. 157]{ibragimov2013statistical}. 

The proof of the regularity of $\bar{\theta}_n$ requires the following refinement of Theorem~\ref{thm:polyak_juditsky}.
