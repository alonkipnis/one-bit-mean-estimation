...
\visible<3->{
\item Next: {\color{red} adaptive: $M_i = f_i(X_i,M^{i-1})$ \hfill{ARE$=\pi/2$} }  }
\end{itemize}
\end{frame}
...
\draw[color = red, line width = 1.5pt] (ti)+(0,0.2) -- node[above, yshift = 0.08cm] {\color{black} $\theta$} +(0,-0.2);
\draw[<-,line width = 2pt] (-1.5,0) -- node[above] {$-1$} (ti);
\draw[->,line width = 2pt] (ti) -- node[above] {$1$} (1.5,0);
\draw (-1.3,1.5) -- (ti) -- (1.3,1.5);
\end{tikzpicture}
\end{flushright}
\[
...
\begin{center}
\begin{tikzpicture}
\draw[<->,line width = 1pt] (-3,0) node[left] {$\cdots$} -- (3,0) node[right] {$\cdots$};
\node[coordinate] at(0,0) (ti) {};
%\draw[<-,line width = 1pt] (-3,0) -- node[above] (ti);
%\draw[->,line width = 1pt] (ti) -- node[above]  (6,0);
\draw[color = red, line width = 1.5pt] (ti)+(0,0.2) -- node[above, yshift = 0.08cm] {\color{black} $t_1$} +(0,-0.2);
\pause
\node[coordinate] at(2,0) (tj) {};
\draw[color = blue, line width = 1.5pt] (tj)+(0,0.2) -- node[above, yshift = 0.08cm] {\color{black} $t_2$} +(0,-0.2);
\pause
\node[coordinate] at(-1,0) (tk) {};
\draw[color = green, line width = 1.5pt] (tk)+(0,0.2) -- node[above, yshift = 0.08cm] {\color{black} $t_3$} +(0,-0.2);
\end{tikzpicture}
\end{center}
%Assume:
%\[
