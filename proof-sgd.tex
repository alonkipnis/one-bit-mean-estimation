...
We begin by recalling some of these concepts.
\begin{definition}
...
\begin{definition}
  ...
  \emph{locally asymptotically normal with information matrix $I_\theta$} at
...
First~\cite[Theorem 7.2]{VanDerVaart98},
if $P_\theta$ is QMD at $\theta$, then
it is locally asymptotically normal with
$Z_n = \frac{1}{\sqrt{n}} \sum_{i = 1}^n \score_\theta(X_i)$ and information
matrix $I_\theta = \E_\theta[\score_\theta \score_\theta^\top]$. Moreover,
in any locally asymptotically normal family at $\theta$,
if $T_n$ is a regular estimator at $\theta$ with limiting distribution $Q$,
then for any bounded, symmetric, quasi-convex loss $L$ and $c < \infty$,
\begin{equation*}
  \limsup_n \sup_{\norm{h} \le c}
  \E_{P_{\theta + h/\sqrt{n}}}
...
Thus, we show three results: first, that $\bar{\theta}_n$ is regular,
second, that the family $P_\theta$ is QMD, and finally, combining the two
results.

We first show that the estimator $\bar{\theta}_n$ is regular in the
following sense: For $\theta \in \Theta$, a sequence $h_n \to h \in \R$ and
$n$ large enough that $\theta + h_n /\sqrt{n} \in \Theta$, let
$P_{\theta,n}$ be a product probability measure on $\R^n$ distributed
i.i.d.\ with density
$f(x - \theta - h_n/\sqrt{n})$. Then
\begin{align}
  \label{eqn:sgd-median-regular}
  \sqrt{n}\left( \bar{\theta}_n - \theta\right)
  \mathop{\cd}_{\Prob_{\theta,n}}
  \normal\left( h,\frac{1}{4 f(\theta)^2}\right),
\end{align}
under $\Prob_{\theta,n}$. To show the convergence~\eqref{eqn:sgd-median-regular} we
use the following refinement of Corollary~\ref{corollary:polyak-juditsky},
whose proof we defer to Section~\ref{proof:normal-expansion}.
%
\begin{corollary}
  \label{corollary:normal-expansion}
  Set $\Delta_i = \theta_i - \theta$ and $\bar{\Delta}_i = \frac{1}{n}
  \sum_{i=1}^n \Delta_i$. Let the conditions of
  Corollary~\ref{corollary:polyak-juditsky} hold, and additionally
  assume the local smoothness condition that there exist
  $0 < \lambda \le 1$ and $K < \infty$ such that
  \begin{equation}
...
We now verify that the setting of Theorem~\ref{thm:sgd} satisfies the
conditions of Corollary~\ref{corollary:polyak-juditsky}. First,
we have the obvious fact that
\begin{equation*}
  |\sgn(z) - \sgn(x + z)|
  \le 2 \cdot \indic{|x| \ge |z|}.
\end{equation*}
Recalling our w.l.o.g.\ assumption that $\theta = \mbox{med}(X) = 0$,
for $\varphi(z) = \sgn(z)$, and $Z = X - \theta = X$ we have
\begin{align*}
  \ex{ \left| \varphi(Z) - \varphi(x + Z) \right| }
  \leq 2 \Prob\left( |Z_1| \le |x|  \right)
  \le 2 f(0) |x| + \lip(f) x^2
\end{align*}
for $x$ near 0, where $\lip(f)$ is the Lipschitz constant of $f$ near
its median. For large $x$ we always have
...
With the convergence~\eqref{eqn:sgd-median-regular},
the limiting efficiency estimate~\eqref{eq:attaining_LAM} is straightforward.
Indeed, the limiting distribution~\eqref{eqn:sgd-median-regular}
guarantees that $\bar{\theta}_n$ is a regular estimator,
...
We recall~\cite[Thm.~7.2]{VanDerVaart98} that if
a family $\{P_\theta\}_{\theta \in \Theta}$ is quadratic mean differentiable
at $\theta$ with score $\score_\theta$, then for any sequence
...
\begin{lemma}[Extension of \cite{VanDerVaart98}, Lemma 7.6]
  ...
  $P_\theta$ is QMD~\eqref{eqn:qmd} at $\theta = \theta_0$ with
  $\score_\theta = \dot{p}_\theta / p_\theta$.
\end{lemma}

Deferring the proof of Lemma~\ref{lemma:qmd} temporarily,
we note that the regularity result~\eqref{eq:normal_expansion_lem}
gives
\begin{equation*}
...
\begin{equation*}
  ...
  \Ncal\left(\frac{-h}{ \psi'(0)} \E_f[\varphi(Z) \score(Z)],
  \frac{\chi(0)}{\psi'(0)^2} \right)
\end{equation*}
under the alternatives $P_{h_n/\sqrt{n}}$, which gives
Corollary~\ref{corollary:normal-expansion}\eqref{item:apply-le-cam}.

All that remains is to return to our proof of Lemma~\ref{lemma:qmd}:

\subsubsection*{Proof of Lemma~\ref{lemma:qmd}}
