...
\begin{corollary}{\cite[Thms. 3 \& 4]{polyak1992acceleration}}
  ...
  Let $\varphi : \R \to \R$ and $\{Z_i\}$ be i.i.d.\ \newtext{zero-mean} random
  variables, and
  \begin{equation*}
  X_i = \theta + Z_i.
  \end{equation*}
...
% AK: Here I changed \pm to \leq and \geq
\begin{equation*}
...
\begin{proof}
  ...
  quantities. 
  \newtext{
  As in the proof of Theorems~2 and~4 in \cite{polyak1992acceleration}, the Robbins-Siegmund Theorem \cite{robbins1971convergence} applies to the increment of $|\Delta_t|^2$ implies that for every 
  ...
  \begin{equation*}
    ...
    ~ \mbox{a.s.}
  \end{equation*}
  by Condition~\eqref{eqn:lazy-gamma}. Since
  \begin{align*}
\left\{ \sum_{i=1}^\infty \frac{|\Delta_i|^{1+\lambda}}{\sqrt{i}} < \infty \subset \left\{ \sup_i |\Delta_i| \leq R \right\} \cap \left\{ \sum_{i=0}^\infty  \right\}
 \right\},
  \end{align*}
  ...
  %\newtext{As Lemma~\ref{lemma:converging-power-sum} implies in particular that $\{\Delta_i\}$ is a.s. bounded, with probability 1 there exists some (random) $R < \infty$ such that $\tau_R = \infty$.} We obtain that 
  \begin{equation*}
    \sum_{i = 1}^\infty \frac{1}{i}
