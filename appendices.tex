...
\]
We note that although the lower bound \eqref{eq:ceo_bound} was derived assuming the optimal allocation of $n$ bits per observation among the encoders, this bound cannot be tightened by considering the CEO MSE while enforcing the condition $R_1=\ldots = R_n = 1$. Indeed, an upper bound for the CEO MSE under the condition $R_1=\ldots = R_n = 1$ follows from \cite{KipnisRini2019}, and leads to
\[
...
\leq \eta_\delta(0)
\]
since $\eta_\delta(x)$ attains its maximum at the origin.
...
\begin{align*}
& g(y_1,\ldots,y_N) \triangleq  \frac{\left| \delta_N(y_1,\ldots,y_k,x^*,y_{k+1}\ldots,y_N) )\right|^{2+\delta}}{
\left|\Delta_N(y_1,\ldots,y_k,x_{i^*},y_{k+1}\ldots,y_N)  \right) \left(1 -\Delta_N(y_1,\ldots,y_k,x^*,y_{k+1}\ldots,y_N))  \right|^{1+\delta}
} .
\end{align*}
The claim in the lemma is obtained by taking $y_i=x_{i'}$ where $i'$ is the $i$th element in $\{1,\ldots,N+1\}\setminus \{i^*\}$. It is therefore enough to prove that 
\[
...
%
The maximal value of $g(y_1,\ldots,y_N)$ is attained for the same $(y_1,\ldots,y_N) \in A_{N+1}(x^*,k)$ that maximizes
\begin{align*}
...
%
The partial derivative of $\log(g)(y_1,\ldots,y_N)$ with respect to $y_i$ within the interior of $A_N(x^*,k)$ is given by
\[
...
if $N$ is even, or
\begin{align*}
& g(y_1,\ldots,y_N) = \frac{\left| s_1f(y_1)+s_{N+1}f(x_{i^*})  \right|^{2+\delta}} { 
\left|  s_1F(y_N) + s_{N+1}F(x_{i^*}) \right|^{1+ \delta} 
\left| 1 - (s_1F(y_1) + s_{N+1}F(x_{i^*})) \right|^{1+ \delta} } 
\end{align*} 
if $N$ is odd and $y_1 < x^*$. Form the induction assumption, the last display is bounded from above by $\eta_\delta(x_{i^*})$. The case where $N$ is odd and $y_1 > x^*$ is similar. 
%
We now consider the possibility that the maximum of $g(y_1,\ldots,y_N)$ is attained at the boundaries of $A_N(x^*,k)$. For boundary points for which $y_i = y_{i+1}$ for some $i$, the contribution of $y_i$ and $y_{i+1}$ to $g(y_1,\ldots,y_N)$ is zero and the induction assumption for $n=N-1$ implies that 
\[
g(y_1,\ldots,y_N) \leq \eta_{\delta}(x^*)
\]
at these points. The remaining boundary points of $A_N(x^*,k)$ are covered by the following cases:
\begin{itemize}
\item[(1)]  $y_N \to -\infty$. 
\item[(2)] $y_1 \to \infty$.
\item[(3)] $y_k = x_{i^*}$.
