...
\begin{equation*}
  B_i =
  \begin{cases}
    \indic{X_i \geq \theta_0} & i = 1,\ldots,n_1, \\
    \indic{X_i \geq T_n}& i={n_1+1,\ldots,n},
  \end{cases}
\end{equation*}
where
\begin{equation*}
  ...
  ~~ \mbox{and} ~~
  \theta_n \defeq
  \theta_{n_1} + F^{-1} \left(\frac{1}{n - n_1}
  \sum_{i = n_1}^n B_i \right).
\end{equation*} 
Then
\begin{thm}
  Assume that as $n\to \infty$, $n_1(n) \rightarrow \infty$ and $n_1 / n \to
  0$. Then
  \begin{align*}
    \sqrt{n} \left( {\theta}_n - \theta  \right)
    \cd  \normal\left( 0, 1/\eta(0) \right).
  \end{align*}
\end{thm}
%
%
\begin{proof}
  ...
  $B_i = \indic{X_i \ge T_n}$ for $i \ge 1$.
  By continuity, we have $T_n \cas \theta$ as $n_1 \to \infty$.
  Let
  $E_n = \E[B_i \mid T_n] = P(X_i \ge T_n)$,
  so that $V_n \defeq \var(B_i \mid T_n) = E_n(1 - E_n)$. Define also the random
  variable
  \begin{equation*}
    Y_n \defeq \frac{1}{\sqrt{n}} \sum_{i = 1}^n \frac{B_i - E_i}{
      \sqrt{E_n (1 - E_n)}},
  \end{equation*}
  and let $F_n(\cdot \mid T_n)$ be its cumulative distribution function.
  Then because
  \begin{equation*}
    \E\left[|B_i - E_n|^3 \mid T_n\right] \le V_n = E_n(1 - E_n)
    ~~ \mbox{we have} ~~
    \E\left[\frac{|B_i - E_n|^3}{(E_n(1 - E_n))^{3/2}} \mid T_n\right]
    \le \frac{1}{\sqrt{V_n}}.
  \end{equation*}
  Then the Berry-Esseen theorem implies that there exists a constant
  $C \le 1$ such that
  \begin{equation*}
    \sup_t \left|F_n(t \mid T_n) - \Phi(t) \right|
    \le \frac{C}{\sqrt{V_n} \sqrt{n}} \wedge 2,
  \end{equation*}
  where $\Phi$ is the standard Gaussian CDF.
  As $V_n \cas \frac{1}{4}$ by definition of the median,
  we have
  ...
  \begin{equation*}
    \sup_t \left|\mathbb{P}(Y_n - \theta \le t) - \Phi(t)\right|
    \le \E\left[
      \sup_t \left|F_n(t \mid T_n) - \Phi(t)\right| \right]
    \to 0,
  \end{equation*}
  which gives the result when combined with Slutsky's lemmas.
\end{proof}
%
