...
Optimal threshold density. The threshold density $\lambda^\star(dt)$ (blue) that maximizes the asymptotic relative efficiency for $f(x) = \Ncal(\theta,\sigma^2)$ and $\theta \in [-T,T]$.
%
...
\end{axis}
\end{tikzpicture}
\caption{\label{fig:minimax_ARE} Minimax ARE versus $\sigma$ for $f(x) = \Ncal(\theta,\sigma^2)$ and $\theta \in \Theta = [-1/2,1/2]$. The dashed curve (green) is the ARE under a uniform threshold density over $\Theta$ given by $K_{\unif}\sigma^2$ of \eqref{eq:uniform_risk}.}.
\end{center}
\end{figure}
