...
Optimal threshold density. The threshold density $\lambda^\star(dt)$ (blue) that maximizes the asymptotic relative efficiency for $f(x)$ the normal desnity with variance $\sigma^2$ and $\Theta= [-1/2,1/2]$. 
%
...
\end{axis}
\end{tikzpicture}
\caption{\label{fig:minimax_ARE} Minimax ARE versus $\sigma$ for $f(x)$ the standard normal density with variance $\sigma^2$ and $\Theta = [-1/2,1/2]$. The dashed curve (green) is the ARE under a uniform threshold density over $\Theta$ given by $K_{\unif}\sigma^2$ of \eqref{eq:uniform_risk}.}.
\end{center}
\end{figure}
