...
Minimax relative efficiency under distributed encoding. ARE versus $\sigma$ for $f(x)$ the standard normal density with variance $\sigma^2$ and $\Theta = [-1/2,1/2]$. The dashed curve (green) is the ARE under a uniform threshold density over $\Theta$ given by $K_{\unif}\sigma^2$ of \eqref{eq:uniform_risk}. \newtext{
