...
$P_\theta(A_i) \ge \Delta_i \sum_{j=1}^{k_i} F(t_{i,j}^-)$ and $1-P_\theta(A_i) \ge \Delta_i \sum_{j=1}^{k_i} F(t_{i,j}^+)$. Therefore, As long as $\sum_{j=1}^{k_i} F(t_{i,j}^-)$ and $\sum_{j=1}^{k_i} F(t_{i,j}^+)$ are bounded away from zero, \ref{assumption:detection-regions}\eqref{item:finite-intervals} holds whenever $ \max_{i \le n} k_i^3 \Delta_i^{-4} = o(n)$. 
